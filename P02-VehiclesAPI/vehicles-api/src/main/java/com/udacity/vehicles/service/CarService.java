...
public class CarService {
    ...
    public Car findById(Long id) {
        ...
        try {
            Location location = mapsClient.getAddress(loc);
            car.setLocation(location);
        } catch (Exception e) {
            throw new MapsNotFoundException();
        }*/

        // short cut of lines from 61 - 67
        // car.setLocation(mapsClient.getAddress(car.getLocation()));

        Optional<Location> optionalLocation = Optional.ofNullable(mapsClient.getAddress(car.getLocation()));
        Location location = optionalLocation.orElseThrow(MapsNotFoundException::new);
        car.setLocation(location);

        Optional<String> optionalPriceClient = Optional.ofNullable(priceClient.getPrice(id));
        String price = optionalPriceClient.orElseThrow(PriceNotFoundException::new);
        car.setPrice(price);
